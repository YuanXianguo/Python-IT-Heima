...
from ihome import db, redis_db, constants
...
def get_area_info():
    ...
    # 尝试从redis中读取数据
    try:
        res_json = redis_db.get("area_info")
    ...
    # 将数据保存到redis中
    try:
        redis_db.setex("area_info", constants.AREA_INFO_REDIS_CACHE_EXPIRES, res_json)
    except Exception as e:
        current_app.logger.error(e)
...
def save_house_image():
    ...
    fileurl = "static/images/houses/%s.jpg" % datetime.strftime(datetime.now(), "%Y%m%d%H%M%S")
    filename = "ihome/" + fileurl
    with open(filename, "wb") as f:
        f.write(image_data)

    # 保存图片信息到数据库中
    house_image = HouseImage(house_id=house_id, url=fileurl)
    db.session.add(house_image)

    # 处理房屋的主图片
    if not house.index_image_url:
        house.index_image_url = fileurl
        db.session.add(house)
    ...
    return jsonify(errno=RET.OK, errmsg="OK", data={"image_url": fileurl})
...
def get_house_index():
    ...
    # 从缓存中尝试获取数据
    try:
        ret = redis_db.get("home_page_data")
    except Exception as e:
        current_app.logger.error(e)
        ret = None
    ...
    if ret:
        ...
        try:
            redis_db.setex("home_page_data", constants.HOME_PAGE_DATA_REDIS_EXPIRES, json_houses)
        except Exception as e:
            current_app.logger.error(e)
...
def get_house_detail(house_id):
    ...
    # 先从redis缓存中获取信息
    try:
        ret = redis_db.get("house_info_%s" % house_id)
    except Exception as e:
        current_app.logger.error(e)
        ret = None
    if ret:
    ...
    try:
        redis_db.setex("house_info_%s" % house_id, constants.HOUSE_DETAIL_REDIS_EXPIRE_SECOND, json_house)
    except Exception as e:
        current_app.logger.error(e)
...
def get_house_list():
    ...
    try:
        resp_json = redis_db.hget(redis_key, page)
    except Exception as e:
        current_app.logger.error(e)
    else:
        if resp_json:
    ...
    if page <= total_page:
        ...
        # 哈希类型
        try:
            # redis_db.hset(redis_key, page, resp_json)
            # redis_db.expire(redis_key, constants.HOUES_LIST_PAGE_REDIS_CACHE_EXPIRES)
